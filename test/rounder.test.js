...
describe("rounder.round()", () => {
  ...
  test("rounding both coordinates", () => {
    precision.set(Number.EPSILON)
    const pt1 = { x: new BigNumber(3), y: new BigNumber(4) }
    const pt2 = { x: new BigNumber(3).plus(new BigNumber(Number.EPSILON)), y: new BigNumber(4).plus(new BigNumber(Number.EPSILON)) }
    expect(precision.snap(pt1)).toEqual(pt1)
    expect(precision.snap(pt2)).toEqual(pt1)
  })
  ...
  test("preseed with 0", () => {
    precision.set(Number.EPSILON)
    const pt1 = { x: new BigNumber(Number.EPSILON).div(new BigNumber(2)), y: new BigNumber(-Number.EPSILON).div(new BigNumber(2)) }
