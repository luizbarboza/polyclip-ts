...
describe("sweep event compare", () => {
  test("favor earlier x in point", () => {
    const s1 = new SweepEvent({ x: new BigNumber(-5), y: new BigNumber(4) })
    const s2 = new SweepEvent({ x: new BigNumber(5), y: new BigNumber(1) })
  ...
  test("then favor earlier y in point", () => {
    const s1 = new SweepEvent({ x: new BigNumber(5), y: new BigNumber(-4) })
    const s2 = new SweepEvent({ x: new BigNumber(5), y: new BigNumber(4) })
    expect(SweepEvent.compare(s1, s2)).toBe(-1)
    expect(SweepEvent.compare(s2, s1)).toBe(1)
  })

  test("then favor right events over left", () => {
    const seg1 = Segment.fromRing({ x: new BigNumber(5), y: new BigNumber(4) }, { x: new BigNumber(3), y: new BigNumber(2) })
    const seg2 = Segment.fromRing({ x: new BigNumber(5), y: new BigNumber(4) }, { x: new BigNumber(6), y: new BigNumber(5) })
  ...
  test("then favor non-vertical segments for left events", () => {
    const seg1 = Segment.fromRing({ x: new BigNumber(3), y: new BigNumber(2) }, { x: new BigNumber(3), y: new BigNumber(4) })
    const seg2 = Segment.fromRing({ x: new BigNumber(3), y: new BigNumber(2) }, { x: new BigNumber(5), y: new BigNumber(4) })
  ...
  test("then favor vertical segments for right events", () => {
    const seg1 = Segment.fromRing({ x: new BigNumber(3), y: new BigNumber(4) }, { x: new BigNumber(3), y: new BigNumber(2) })
    const seg2 = Segment.fromRing({ x: new BigNumber(3), y: new BigNumber(4) }, { x: new BigNumber(1), y: new BigNumber(2) })
    expect(SweepEvent.compare(seg1.leftSE, seg2.rightSE)).toBe(-1)
    expect(SweepEvent.compare(seg2.rightSE, seg1.leftSE)).toBe(1)
  })
  ...
  test("then favor lower segment", () => {
    ...
    const seg2 = Segment.fromRing({ x: new BigNumber(0), y: new BigNumber(0) }, { x: new BigNumber(5), y: new BigNumber(6) })
    expect(SweepEvent.compare(seg1.leftSE, seg2.rightSE)).toBe(-1)
    expect(SweepEvent.compare(seg2.rightSE, seg1.leftSE)).toBe(1)
  })
  ...
  test("then favor lower ring id", () => {
    ...
    const seg2 = Segment.fromRing({ x: new BigNumber(0), y: new BigNumber(0) }, { x: new BigNumber(5), y: new BigNumber(5) }, { id: 2 })
    expect(SweepEvent.compare(seg1.leftSE, seg2.leftSE)).toBe(-1)
    expect(SweepEvent.compare(seg2.leftSE, seg1.leftSE)).toBe(1)
  })

  test("identical equal", () => {
    const s1 = new SweepEvent({ x: new BigNumber(0), y: new BigNumber(0) })
    const s3 = new SweepEvent({ x: new BigNumber(3), y: new BigNumber(3) })
  ...
  test("totally equal but not identical events are consistent", () => {
    const s1 = new SweepEvent({ x: new BigNumber(0), y: new BigNumber(0) })
    const s2 = new SweepEvent({ x: new BigNumber(0), y: new BigNumber(0) })
    const s3 = new SweepEvent({ x: new BigNumber(3), y: new BigNumber(3) })
    new Segment(s1, s3, { id: 1 })
  ...
  test("events are linked as side effect", () => {
    const s1 = new SweepEvent({ x: new BigNumber(0), y: new BigNumber(0) })
    const s2 = new SweepEvent({ x: new BigNumber(0), y: new BigNumber(0) })
    new Segment(s1, new SweepEvent({ x: new BigNumber(2), y: new BigNumber(2) }))
    new Segment(s2, new SweepEvent({ x: new BigNumber(3), y: new BigNumber(4) }))
...
describe("sweep event link", () => {
  ...
  test("link events already linked with others", () => {
    const p1 = { x: new BigNumber(1), y: new BigNumber(2) }
    ...
    new Segment(se1, new SweepEvent({ x: new BigNumber(5), y: new BigNumber(5) }))
    new Segment(se2, new SweepEvent({ x: new BigNumber(6), y: new BigNumber(6) }))
    new Segment(se3, new SweepEvent({ x: new BigNumber(7), y: new BigNumber(7) }))
    new Segment(se4, new SweepEvent({ x: new BigNumber(8), y: new BigNumber(8) }))
    se1.link(se3)
...
describe("sweep event get leftmost comparator", () => {
  test("after a segment straight to the right", () => {
    const prevEvent = new SweepEvent({ x: new BigNumber(0), y: new BigNumber(0) })
    const event = new SweepEvent({ x: new BigNumber(1), y: new BigNumber(0) })
    const comparator = event.getLeftmostComparator(prevEvent)

    const e1 = new SweepEvent({ x: new BigNumber(1), y: new BigNumber(0) })
    new Segment(e1, new SweepEvent({ x: new BigNumber(0), y: new BigNumber(1) }))

    const e2 = new SweepEvent({ x: new BigNumber(1), y: new BigNumber(0) })
    new Segment(e2, new SweepEvent({ x: new BigNumber(1), y: new BigNumber(1) }))

    const e3 = new SweepEvent({ x: new BigNumber(1), y: new BigNumber(0) })
    new Segment(e3, new SweepEvent({ x: new BigNumber(2), y: new BigNumber(0) }))

    const e4 = new SweepEvent({ x: new BigNumber(1), y: new BigNumber(0) })
    new Segment(e4, new SweepEvent({ x: new BigNumber(1), y: new BigNumber(-1) }))

    const e5 = new SweepEvent({ x: new BigNumber(1), y: new BigNumber(0) })
    new Segment(e5, new SweepEvent({ x: new BigNumber(0), y: new BigNumber(-1) }))
  ...
  test("after a down and to the left", () => {
    const prevEvent = new SweepEvent({ x: new BigNumber(1), y: new BigNumber(1) })
    const event = new SweepEvent({ x: new BigNumber(0), y: new BigNumber(0) })
    const comparator = event.getLeftmostComparator(prevEvent)

    const e1 = new SweepEvent({ x: new BigNumber(0), y: new BigNumber(0) })
    new Segment(e1, new SweepEvent({ x: new BigNumber(0), y: new BigNumber(1) }))

    const e2 = new SweepEvent({ x: new BigNumber(0), y: new BigNumber(0) })
    new Segment(e2, new SweepEvent({ x: new BigNumber(1), y: new BigNumber(0) }))

    const e3 = new SweepEvent({ x: new BigNumber(0), y: new BigNumber(0) })
    new Segment(e3, new SweepEvent({ x: new BigNumber(0), y: new BigNumber(-1) }))

    const e4 = new SweepEvent({ x: new BigNumber(0), y: new BigNumber(0) })
    new Segment(e4, new SweepEvent({ x: new BigNumber(-1), y: new BigNumber(0) }))
